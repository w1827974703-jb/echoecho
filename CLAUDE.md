...
### 存储说明与后续升级路径
- **音频二进制**存浏览器 **IndexedDB**（`lib/audioStore.ts`，key = `AudioItem.id`）：刷新/重开/点历史都能回放；转录用的 OSS 临时对象转录后即删。
- **字幕/生词**等元数据存 localStorage（`lib/store.ts`）。
- 以上均为**单浏览器本地**，不跨设备、不多用户。
- **后续大版本（上线用）**：账号系统 + 云数据库（如 Supabase）+ 音频云存储，实现跨设备同步与多用户。这是独立的架构升级，不在一周 MVP 范围内。
